...
def SVD():
    ...
    #-----------A_A_transpose--------------
    def A_to_A_transpose(a_list):
        A_A_transpose_list = mat_mul(a_list,transpos(a_list))
        return A_A_transpose_list
    print("This is our A_to_A_Transpose matrix" , A_to_A_transpose(a_list),"\n")
    ...
    #-----------A_A_transpose - xI --------------
    def A_A_transpose_sub_xI(a_list,matrix_size):
        A_A_transpose_sub_xI_list = mat_substract(A_to_A_transpose(a_list),xI_matrix(matrix_size))
        return A_A_transpose_sub_xI_list
    print(A_A_transpose_sub_xI(a_list,matrix_size),"This is AAt - xI","\n")
    ...
    print(A_transpose_A_sub_xI(a_list,matrix_size)," This is AtA - xI","\n")
    ...
    if matrix_size == 3 :
        ...
        AAt_sub_xI1 = np.array(mat_substract(A_to_A_transpose(a_list),xI1_np))
        AAt_sub_xI2 = np.array(mat_substract(A_to_A_transpose(a_list),xI2_np))
        AAt_sub_xI3 = np.array(mat_substract(A_to_A_transpose(a_list),xI3_np))
    elif matrix_size ==2 :
        xI1_np = make_format(xI1_list)
        xI2_np = make_format(xI2_list)
        AAt_sub_xI1 = np.array(mat_substract(A_to_A_transpose(a_list),xI1_np))
    ...
    #-------------CEF of matrix AtA-xI and AAt-xI-------------------
    def ref(mtx): 
    ...
    #-----------Eigenvectors for AAt---------------------
    w1 , vAAt = np.linalg.eig(A_to_A_transpose(a_list))
    #print(vAAt," Eigenvector matrix for AAt matrix","\n")

    #-----------Eigenvectors AtA---------------------
    w2 , vAtA = np.linalg.eig(A_transpose_to_A(a_list))
    #print(vAtA," Eigenvector matrix for AtA matrix","\n")

    #----------Format for Orto 3x3 to 1x9 -------------
    def format_for_Orto(mat,matrix_size):
        l = []
        for i in range(0,matrix_size):
            for j in range(0,matrix_size):
                l.append(mat[i][j])
        return l
    v_Orto_AAt = format_for_Orto(vAAt,matrix_size)
    v_Orto_AtA = format_for_Orto(vAtA,matrix_size)

    def orthogonal(a):
        def size(a_list):
            x = len(a_list)
            y = int(np.size(a_list)/x)
            return x,y
    ...
    #-----------Z5 -------------
    def matrix_by_z5(mat):
        for i in range(0,matrix_size):
            for j in range(0,matrix_size):
                mat[i][j] = round(mat[i][j] * 36) % 5 
        return mat
    #-------round --------------
    def rou(mat):
        for i in range(0,matrix_size):
            for j in range(0,matrix_size):
                mat[i][j] = round(mat[i][j],4)
        return mat
    #-----------------U ----------------------
    Orto_AAt_matrix =transpos(orthogonal(v_Orto_AAt))
    #print(Orto_AAt_matrix , "This is Ortogonalized AAt matrix " , "\n")

    #------------------V^T---------------------
    Transpo_Orto_AtA_matrix = orthogonal(v_Orto_AtA)
    #print(Transpo_Orto_AtA_matrix , " This is Ortogonalized Transposed AtA matrix " , "\n")
    ...
    Orto_AAt_matrix_by_z5 = matrix_by_z5(Orto_AAt_matrix)
    Transpo_Orto_AtA_matrix_by_z5 = matrix_by_z5(Transpo_Orto_AtA_matrix)
    Square_diagonal_matrix_by_z5 = matrix_by_z5(Square_diagonal_list)

    #print(Orto_AAt_matrix_by_z5)
    #print(Transpo_Orto_AtA_matrix_by_z5)
    #print(Square_diagonal_matrix_by_z5)

    return(mat_mul(mat_mul(Orto_AAt_matrix,Square_diagonal_matrix_by_z5),Transpo_Orto_AtA_matrix_by_z5))
